...
| **hoda-member-assistant** | ゼミ生お助けエージェント。部署別の実務のコツと、授業運営部が作る2つのシート（発表FBシート／輪読マテリアル選定シート）のテンプレ生成 |
...
| **hoda-member-assistant** | 部署別の実務のコツ。現在は授業運営部を収録（イベント運営部・広報部・会計は未収録） |
